...
class PrimEnv(gym.Env):
    ...
    # valid_mask = [0 0 0 1 ... 1 0 0 0] (action_num)
    # 1 表示可以的动作集合
    # box_id == self.step_count % self.box_num -> 也就是按顺序逐个处理box
    # 如果已经被删除，就不存在delete action
    # edit操作需要在范围内操作
    ...
    def add_edgeloop_to_all(self, boxlist):
        ...
        for i in range(boxnum):
            ...
            v, f, l = self.add_edgeloop_to_one_box(
                boxlist[i][0:3], boxlist[i][3:6], box_subdiv[i, 0], box_subdiv[i, 1], box_subdiv[i, 2], v_offset=offset, l_offset=loop_offset)
    ...
    def output_result(self, log_info, save_path):
        ...
        if self.step_count == self.max_step:
            self.cleaned_boxes = utils.merge_boxlist(
                self.cleaned_boxes, self.vox_size_l, self.box_num, 0.85, True)
            self.cleaned_boxes = utils.merge_boxlist(
                self.cleaned_boxes, self.vox_size_l, self.box_num, 0.9, True)
        ...
        if self.cleaned_boxes.shape[0] != 0:
            ...
            self.scaled_box_list = utils.get_scaled_prim_point(
                self.cleaned_boxes, self.target_points, self.target_h_points)

            # avoid overlapping mesh for better visualization
            r_scaled_box_list = self.scaled_box_list + \
                np.random.random(self.scaled_box_list.shape)*0.1
    ...
    def save_edgeloop(self, save_path):
        ...
        vertex_list, face_list, control_v_list, loop_list, loop_map, box_loop_list = self.add_edgeloop_to_all(
            self.scaled_box_list)
        ...
        with open(loop_info_path, "w") as file:
            file.write(str(len(control_v_list))+" " +
                       str(len(box_loop_list))+"\n")
            ...
            file.write(
                '# loop direction, loop id, the id of the first control point in the primitive, the id of the second one\n')
            for i in range(len(loop_map)):
                file.write(str(loop_map[i][0])+" "+str(loop_map[i][1]) +
                           " "+str(loop_map[i][2])+" "+str(loop_map[i][3])+"\n")
