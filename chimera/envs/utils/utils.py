...
def save_point_off(points, colors, path, use_color=True):
    with open(path, "w") as file:
        ...
        for i in range(points.shape[0]):
            file.write(str(float(points[i][0])) + " " + str(
                float(points[i][1])) + " " + str(float(points[i][2])) + " ")
            if use_color == False:
                file.write("150 0 0\n")
            else:
                file.write(str(colors[0]) + " " +
...
def save_mesh_obj(vertexlist, facelist, path):
    with open(path, "w") as file:
        ...
        for i in range(vertexlist.shape[0]):
            file.write("v "+str(vertexlist[i][0]) + " " + str(
                vertexlist[i][1]) + " " + str(vertexlist[i][2]) + "\n")
...
def merge_boxlist(boxlist, canvas_size, max_box_num, merge_t, keep_color=False):
    ...
    for i in range(boxlist.shape[0]):
        ...
        for j in range(i+1, boxlist.shape[0]):
            box_j = np.zeros(
                (canvas_size, canvas_size, canvas_size), dtype=np.int)
    ...
    with open(path, "w") as file:
        ...
        for i in range(boxnum):
            for j in range(12):
                ...
                file.write(str(int(
                    colors[i][0])) + " " + str(int(colors[i][1])) + " " + str(int(colors[i][2])) + "\n")
...
def get_scaled_prim_point(boxlist, source_v, target_v):
    ...
    box_v1 = scale_vertex_by_bbox(
        box_v1, c_source_1, c_source_2, c_target_1, c_target_2)
    box_v2 = scale_vertex_by_bbox(
        box_v2, c_source_1, c_source_2, c_target_1, c_target_2)
...
def rand_sample_points_on_tri_mesh(vertices, faces, num_sample):
    ...
    weighted_random_indices = np.random.choice(
        range(areas.shape[0]), size=num_sample, p=probabilities)
...
def triangulation_quad_mesh(face):
    ...
    for i in range(face.shape[0]):
        tri_face[i*2] = np.array([face[i][3], face[i]
                                 [2], face[i][1]], np.int)-1
        tri_face[i*2+1] = np.array([face[i][1], face[i]
                                   [0], face[i][3]], np.int)-1
    return tri_face
