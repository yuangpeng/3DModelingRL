...
class Net(nn.Module):
    def __init__(self, ):
        ...
        self.boxfc1 = nn.Linear(p.LOOP_NUM*8, 128)
    ...
    def forward(self, x, boxx, stepx):
        ...
        x = torch.cat((x, boxx, stepx), dim=1)
...
class Agent(object):
    ...
    def choose_action(self, x, box, step, valid_map, greedy=1.0):

        x = torch.unsqueeze(torch.FloatTensor(x), 0)
        box = torch.unsqueeze(torch.FloatTensor(box), 0)
        step = torch.unsqueeze(torch.FloatTensor(step), 0)
        ...
        if np.random.uniform() < greedy:
            ...
            valid_map = torch.from_numpy(valid_map).float()
            valid_map = valid_map.unsqueeze(0)

            actions = actions*valid_map
            ...
            valid_actions = np.array(valid_map.nonzero(), dtype=np.int)[0]
    ...
    def learn(self, learning_mode, is_ddqn=True):
        ...
        bbox = torch.FloatTensor(np.concatenate((bbox1, bbox2), axis=0))
        bstep = torch.FloatTensor(np.concatenate(
            (bstep1, bstep2), axis=0))

        ba = torch.FloatTensor(np.concatenate(
            (ba1, ba2), axis=0)).long()
        br = torch.FloatTensor(np.concatenate((br1, br2), axis=0))

        bs_ = torch.FloatTensor(np.concatenate((bs_1, bs_2), axis=0))
        bbox_ = torch.FloatTensor(np.concatenate(
            (bbox_1, bbox_2), axis=0))
        bstep_ = torch.FloatTensor(np.concatenate(
            (bstep_1, bstep_2), axis=0))
        ...
        margin = torch.FloatTensor(
            np.ones((p.BATCH_SIZE, p.ACTION_NUM)))*0.8
        ...
        if not is_ddqn:
            ...
            q_a_target = br + p.GAMMA * \
                q_a_next.max(1)[0].view(p.BATCH_SIZE, 1)
            loss2 = self.loss_func(q_a_expert, q_a_target)

        else:
            q_a_next = self.target_net(bs_, bbox_, bstep_).detach()
            best_action = self.eval_net(bs_, bbox_, bstep_).max(1)[
                1].view(p.BATCH_SIZE, 1)
