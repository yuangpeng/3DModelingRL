...
class Environment():
    ...
    def __init__(self):
        ...
        self.target = np.zeros(
            (1, self.vox_size, self.vox_size, self.vox_size))
    ...
    # shape_infopack = ['demo', 'eval/demo-16.binvox', 'eval/demo-64.binvox',
    # 'prim_result/demo.obj', 'prim_result/demo.loop', 'rgb', 'demo.png']
    def reset(self, shape_infopack):
        ...
        valid, self.ctrl_v, self.loop, self.loop_map, self.box_loop = self.load_loop(
            loop_info_fn)
    ...
    def compute_IOU(self, prim_v):
        ...
        prim_v = ((prim_v+self.transform_move) *
                  self.transform_scale).astype(np.int)
    ...
    def edit_loop(self, action):
        ...
        new_selected_loop_v = utils.scale_vertex_by_bbox(
            old_selected_loop_v, old_keyv[0], old_keyv[1], new_keyv[0], new_keyv[1])
    ...
    def eval_CD_error(self, output_v, output_f, GT_filename):
        ...
        initial_v = utils.rand_sample_points_on_tri_mesh(
            self.init_prim_v, output_f, 4000)
        initial_v = utils.normalize_points(initial_v)

        sample_v = utils.rand_sample_points_on_tri_mesh(
            output_v, output_f, 4000)
