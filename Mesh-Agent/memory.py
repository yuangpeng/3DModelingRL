...
class Memory():
    ...
    def __init__(self, capacity):
        ...
        self.obj_mem = np.zeros((self.capacity, self.loop_num, 2, 4))
        self.step_mem = np.zeros((self.capacity, p.MAX_STEP))

        self.a_mem = np.zeros((self.capacity, 1))
        self.r_mem = np.zeros((self.capacity, 1))

        self._s_mem = np.zeros(
            (self.capacity, 1, self.ref_size, self.ref_size))
        self._obj_mem = np.zeros((self.capacity, self.loop_num, 2, 4))
